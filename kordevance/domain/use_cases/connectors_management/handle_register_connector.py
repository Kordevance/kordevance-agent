...
class HandleRegisterConnector(UseCase[ConnectorRequest, str]):
    ...
    async def execute(self, request: ConnectorRequest) -> str:
        self._logger.info(
            f"Registering connector {request.provider}-{request.category} for profile {request.profile_id}"
        )
        return await self._service.register_connector(request.profile_id, request.provider, request.category)
