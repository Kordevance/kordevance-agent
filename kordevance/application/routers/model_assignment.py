...
async def set_model_assignment(
    ...
    payload = SetModelAssignmentRequest(
        profile_id=profile_id, role=role, provider_id=body.provider_id, model_id=body.model_id
    )
...
async def delete_model_assignment(
    role: ModelRole, service: DeleteModelAssignmentUseCaseDep, profile_id: UUID = Depends(get_profile_id)
) -> None:
    await service.execute(DeleteModelAssignmentRequest(profile_id=profile_id, role=role))


@router.get("")
async def get_model_assignments(
    service: GetModelAssignmentsUseCaseDep, profile_id: UUID = Depends(get_profile_id)
) -> list[ModelAssignmentResponse]:
    assignments = await service.execute(profile_id)
